...
def plot_shape_factor(x,H):
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(x_values, H, marker='o', linestyle='-', color='green', label=r'$\H$')
    plt.xlabel("x-location")
...
def plot_bl_thickness(x,boundary_layer_indeces):
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(x_values, boundary_layer_indeces[1], marker='o', linestyle='-', color='b', label=r'$\H$')
    plt.xlabel("x-location")
    plt.ylabel("bl Thickness")
    plt.title("bl Thickness vs. x-location")
    plt.grid(True)
    plt.legend()
    tas.send_plot('integral_plots')
